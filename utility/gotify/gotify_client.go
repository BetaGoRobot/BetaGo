...
func SendMessage(ctx context.Context, title, msg string, priority int) {
	...
	defer span.End()
	logs.L.Info(ctx, "send message", "traceID", span.SpanContext().TraceID().String())
	...
	if err != nil {
		logs.L.Error(ctx, "could not send message", "error", err)
		return
	}
	logs.L.Info(ctx, "Gotify Message Sent!")
}
