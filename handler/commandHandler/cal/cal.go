...
func GetUserChannelTimeMap(ctx context.Context, userID, guildID string) map[string]time.Duration {
	...
	if err != nil {
		errorsender.SendErrorInfo(consts.NotifierChanID, "", userInfo.ID, err, ctx)
		return nil
	}
	...
	for _, log := range logs {
		leftTimeT, _ := time.Parse(consts.TimeFormat, log.LeftTime)
		joinTimeT, _ := time.Parse(consts.TimeFormat, log.JoinedTime)
...
func DrawPieChartWithAPI(ctx context.Context, inputMap map[string]time.Duration, userName string) (string, error) {
	...
	resp, err := requests.Req().Get(apiURL)
	if err != nil {
		return "", err
	}
	data := resp.Body()
	...
	filePath := filepath.Join(consts.ImagePath, time.Now().Format(time.RFC3339)+"_"+userName+"_chtime.png")
...
func DrawPieChartWithLocal1(ctx context.Context, inputMap map[string]time.Duration, userName string) (string, error) {
	...
	filePath := filepath.Join(consts.ImagePath, fileName)
	f, _ := os.OpenFile(filePath, os.O_CREATE|os.O_RDWR|os.O_TRUNC, 0o666)
	defer f.Close()
...
func DrawPieChartWithLocal(ctx context.Context, inputMap map[string]time.Duration, userName string) (string, error) {
	...
	resp, err := consts.HttpClient.R().SetBody(fullStr).Post("http://betago-server-ix-chart.ix-betago-server:8088/make-charts")
	if err != nil {
		return "", err
	}
