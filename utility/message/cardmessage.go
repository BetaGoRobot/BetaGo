...
import (
	...
	arkResponses "github.com/BetaGoRobot/BetaGo/utility/ark/responses"
...
func SendAndUpdateStreamingCard(ctx context.Context, msg *larkim.EventMessage, msgSeq iter.Seq[*arkResponses.ModelStreamRespReasoning]) (err error) {
...
func SendAndReplyStreamingCard(ctx context.Context, msg *larkim.EventMessage, msgSeq iter.Seq[*arkResponses.ModelStreamRespReasoning], inThread bool) (err error) {
	ctx, span := otel.LarkRobotOtelTracer.Start(ctx, reflecting.GetCurrentFunc())
	defer span.End()
	defer func() { span.RecordError(err) }()
...
func updateCardFunc(ctx context.Context, res iter.Seq[*arkResponses.ModelStreamRespReasoning], cardID string) (err error, lastIdx int) {
	ctx, span := otel.LarkRobotOtelTracer.Start(ctx, reflecting.GetCurrentFunc())
	defer span.End()
	defer func() { span.RecordError(err) }()
	idx := &atomic.Int32{}
	...
	eg.Go(func() error {
		...
		writeFunc := func(data arkResponses.ModelStreamRespReasoning) error {
	...
	clearQueue := func() {
		if len(chunkQueue) > 0 {
			for key, content := range chunkQueue {
				if key == "content" {
					// 尝试修复 JSON 字符串
					contentStruct := &arkResponses.ContentStruct{}
