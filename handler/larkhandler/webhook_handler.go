...
func GetCardMusicByPage(ctx context.Context, musicID string, page int) *templates.TemplateCardContent {
	...
	if err != nil {
		logs.L.Error(ctx, "get music url error", "error", err.Error())
		return nil
	}
	...
	if err != nil {
		logs.L.Error(ctx, "upload pic error", "error", err.Error())
		return nil
	}
	...
	u, err := miniohelper.Client().
		...
		Upload()
	if err != nil {
		logs.L.Error(ctx, "upload to minio error", "error", err.Error())
		return nil
	}
...
func SendAlbumCard(ctx context.Context, albumID string, msgID string) {
	...
	if err != nil {
		logs.L.Error(ctx, "get album detail error", "error", err.Error())
		return
	}
...
func HandleWithDraw(ctx context.Context, cardAction *callback.CardActionTriggerEvent) {
	...
	if consts.WITHDRAW_REPLACE {
		...
		if err != nil {
			logs.L.Error(ctx, "patch card error", "error", err.Error())
		}
	} else {
		// 撤回消息
		resp, err := lark.LarkClient.Im.Message.Delete(ctx, larkim.NewDeleteMessageReqBuilder().MessageId(msgID).Build())
		if err != nil {
			logs.L.Error(ctx, "delete message error", "error", err.Error())
			return
		}
		if !resp.Success() {
			logs.L.Error(ctx, "delete message error", "error", resp.Error())
		}
	}
}
...
func HandleRefreshMusic(ctx context.Context, musicID, msgID string) {
	...
	if !resp.Success() {
		logs.L.Error(ctx, "refresh music card error", "error", err)
		return
	}
	return
}
...
func HandleRefreshObj(ctx context.Context, cardAction *callback.CardActionTriggerEvent) {
	...
	*data.Event.Message.ChatId = cardAction.Event.Context.OpenChatID
	...
	if err != nil {
		logs.L.Error(ctx, "refresh obj error", "error", err)
	}
}
...
func HandleSubmit(ctx context.Context, cardAction *callback.CardActionTriggerEvent) {
	...
	if err != nil {
		logs.L.Error(ctx, "parse start time error", "error", err.Error())
	}
	et, err := time.ParseInLocation("2006-01-02 15:04 -0700", etStr, utility.UTCPlus8Loc())
	if err != nil {
		logs.L.Error(ctx, "parse end time error", "error", err.Error())
	}
