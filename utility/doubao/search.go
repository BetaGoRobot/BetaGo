...
// HybridSearch 执行混合搜索
func HybridSearch(ctx context.Context, req HybridSearchRequest) (searchResults []*SearchResult, err error) {
	if req.TopK <= 0 {
		req.TopK = 5
	}
	...
	for _, query := range req.QueryText {
		...
		queryVec, _, err = EmbeddingText(ctx, query)
		if err != nil {
	...
	log.Println(sonic.MarshalString(query))
	...
	for _, hit := range res.Hits.Hits {
		result := &SearchResult{}
		err = sonic.Unmarshal(hit.Source, &result)
		if err != nil {
			return
		}
